...
def generate_controller(name: str, class_name: str, nodes: list[dict[str, object]]) -> str:
    keys = ["root"] + [str(node["property"]) for node in nodes]
    key_union = " | ".join(q(key) for key in keys)
    refs_lines = ["  root: UguiNode;"] + [f"  {node['property']}?: UguiNode;" for node in nodes]
    button_nodes = [node for node in nodes if node.get("role") == "button"]

    wire_lines = []
    for node in button_nodes:
        key = str(node["property"])
        method = click_handler_name(key)
        wire_lines.append(f'    this.tryBindClick("{key}", bindClick, () => this.{method}());')
    if not wire_lines:
